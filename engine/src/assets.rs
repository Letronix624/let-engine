...
//! access the asset data using a function provided here.
//!
//! ## Basic usage
//!
...
//! - Access the data using the `group_name` function with the group root relative resource path.
//! - Inspect lower ram usage in a system monitor application.
//!
//! ## Groups
//!
//! "Groups" are user defined asset categories that you can give their own settings like compression
//! and easily access here using a function generated by the build script with the group name.
//!
...
//!
//! `texture("environment/stone.png")`
//!
//! `sound("monsters/zombie/growl.oga")`
//!
...
impl Compression {
    ...
    pub fn decompress(&self, buffer: &[u8]) -> Result<Vec<u8>> {
        ...
        match self {
            ...
            #[cfg(feature = "lz4")]
            Compression::Lz4 => {
                let decoder = lz4::Decoder::new(buffer)?;
                let result = decoder.finish();
                decompressed = result.0.to_vec();
                result.1?;
            }
        }
        Ok(decompressed)
